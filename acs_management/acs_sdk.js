...
/*
$.getScript("acs_management/mcms_basic/vars.js", function(){
...
function sdk_user_info(msg){
  sdk.sendRequest('users/show/me.json', 'GET', null, function(responseData) {
    if (responseData && responseData.meta && responseData.meta.code == 200) {
		  ...
		  sdk.sendRequest('photos/show.json', 'GET', data, function (responseData) {
			if (responseData && responseData.meta && responseData.meta.code == 200) {
				...
				var span = document.createElement('span');
					  //span.innerHTML = ['<img class="filequeuethumb" src="../scripts/', custome_photo.urls['thumb_100'],
	  ...
	  $('#loginstatus').html('Welcome, '+useremail+' ; <a href="../scripts/login.php">Log Out</a>');
      return true;
    } else {
      if (msg){alert(responseData.meta.message);}
	  return false;
...
function sdk_user_login(user_account,user_password,ref_location){
...
function sdk_user_create(user_email,user_firstname,user_lastname,user_password,user_password_confirmation,msg,user_username,ref_location,user_role){
  ...
  sdk.sendRequest('users/create.json', 'POST', data, function(responseData) {
    if(responseData && responseData.meta && responseData.meta.code == 200) {
			var contetn_UserID= responseData.response.users[0].id;
			//create first object - TabGroup
				...
				sdk.sendRequest('objects/windows/create.json', 'POST', firstdata, function (responseData) {
				if (responseData && responseData.meta && responseData.meta.code == 200) {
									...
									sdk.sendRequest('users/logout.json', 'GET', null, function(responseData){
										if(responseData && responseData.meta && responseData.meta.code == 200) {
											...
											//create viewer account
												...
												role:user_role,
												custom_fields:'{"content_user_id":"'+contetn_UserID+'"}'
												};	  
												
												sdk.sendRequest('users/create.json', 'POST', viewerdata, function(responseData){
													 if (responseData && responseData.meta && responseData.meta.code == 200) {
														var message = '';
														var user = responseData.response.users[0];
														...
														//after create viewer account is done, return to the modify user account
														sdk_user_login(user_username,user_password,ref_location)
														//window.location=ref_location;
													} else {
														alert(responseData.meta.message)
													}		  	  
...
function sdk_user_update_role(user_email,user_password,user_role){

  sdk.sendRequest('users/login.json', 'POST', {login:user_email, password:user_password}, function(responseData) {
    if (responseData && responseData.meta && responseData.meta.code == 200) {
      //var user = responseData.response.users[0];
      //useremail = user.email
			var data = {
...
function sdk_item_list(userid,win_id_reg){
	var data = {
	  per_page:20,
	  order: '-win_id',
	  where: '{"user_id":"'+userid+'","win_id":{"$ne":0,"$lt":999}, "win_root_id":'+win_id_reg+'}',
	  //","win_id":{"$ne":0}
	};
	sdk.sendRequest('objects/windows/query.json', 'GET', data, function (responseData) {
	  if (responseData && responseData.meta && responseData.meta.code == 200) {
		...
		for (var i=itemcount; i>=0;i--){
			...
			//alert('Win ID:'+custome_window.win_id+'Type:'+custome_window.win_type)
			if (custome_window.win_type == "TYPE_TABLE"){
				//add_pages_row(i,custome_window)
				add_pages_row(custome_window.win_id,custome_window,userid,custome_window.win_id)
	...
	sdk.sendRequest('photos/query.json', 'GET', data, function (responseData) {
	  if (responseData && responseData.meta && responseData.meta.code == 200) {
		...
		for (var i=itemcount; i>=0;i--){
			...
			//$(".jTscroller").append('<a href="#"><img src="../scripts/'+photoitem.urls['thumb_100']+'" /></a>')
